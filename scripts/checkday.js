...
function calcVo2() {
	...
	if ([age, ht, wt, hr].some(isNaN)) {
		document.getElementById("vo2-result").style.display = "none";
		return;
	}
	const vr = calcVo2Value(age, ht, wt, hr);
	const gradeInfo = getVo2Grade(vr, age);
	const style = VO2_GRADE_STYLES[gradeInfo.grade];
	document.getElementById("vo2-val").textContent =
		vr.toFixed(1) + " ml/kg/min";
...
function buildEvals() {
	...
	evals.forEach((e, i) => {
		...
		const dots = [0, 1, 2, 3]
			.map((j) => `<div class="dot" id="dot-${i}-${j}"></div>`)
			.join("");
		const tags = e.checks
			.map(
				(ch) =>
...
function adj(i, d) {
	...
	for (let j = 0; j < 4; j++)
		document
			.getElementById(`dot-${i}-${j}`)
			.classList.toggle("on", j < scores[i]);
	updateTotal();
}
...
function resetAll() {
	...
	evals.forEach((_, i) => {
		...
		for (let j = 0; j < 4; j++)
			document.getElementById(`dot-${i}-${j}`).classList.remove("on");
	});
	[
		"tag-w",
		"tag-m",
...
function getIbData() {
	return {
		w: document.getElementById("ib-w").value,
		m: document.getElementById("ib-m").value,
		fat: document.getElementById("ib-fat").value,
		bmi: document.getElementById("ib-bmi").value,
		bfp: document.getElementById("ib-bfp").value,
		bmr: document.getElementById("ib-bmr").value,
...
function copyReport() {
	...
	const lines = [
		...
		document.getElementById("ib-comment").value
			? `  코멘트: ${document.getElementById("ib-comment").value}`
			: "",
		`━ 움직임 총점: ${tot}/24점`,
		...evalLines,
		`━ 다음 목표: ${goals || "미선택"}`,
		...
		...fbLines,
		document.getElementById("consult-memo").value
			? `━ 상담 메모: ${document.getElementById("consult-memo").value}`
			: "",
	].filter((l) => l !== "");
	navigator.clipboard
