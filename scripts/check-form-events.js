...
export function setupCheckFormEvents() {
	...
	const ibGrid = UI.byId("ib-grid");
	if (ibGrid) ibGrid.innerHTML = TPL.inbodyGrid();
	const goalTags = UI.byId("goal-tags");
	...
	UI.delegate(document, "click", ".goal-tag, .ctag", (e, el) => toggleTag(el));
	UI.delegate(document, "keydown", ".goal-tag, .ctag", (e, el) => {
