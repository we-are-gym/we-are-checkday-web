...
function attachCdp(ws) {
  ...
  ws.addEventListener("message", (ev) => {
    ...
    if (msg.method === "Runtime.exceptionThrown") {
      ...
      s.errors.push(
        `예외: ${ex.text} ${ex.exception?.description || ""}`.trim(),
      );
    }
    if (
      ...
      const args = msg.params.args
        ...
        .join(" ");
      if (/error|uncaught|is not|undefined|failed/i.test(args))
        s.errors.push(`콘솔: ${args}`);
    }
...
async function main() {
  ...
  try {
    ...
    for (const { file, query, markers } of PAGES) {
      ...
      if (!ok && missing.length && !domEmpty)
        console.log(`   DOM조각: ${dom.slice(0, 200).replace(/\\s+/g, " ")}`);
    }
    ws.close();
