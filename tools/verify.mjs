...
// TS 모듈(.ts)은 브라우저가 실행할 수 없으므로, Bun.Transpiler로 구문만 변환해 서빙한다. (bun tools/verify.mjs 실행)
...
const PAGES = [
  ...
  {
    file: "check-doc-view.html",
    query: "docID=1",
    markers: ["vh-title", "ib-tag"],
  },
...
/** .ts 구문 변환 전용 트랜스파일러 (Bun.Transpiler — loader: ts) */
...
const server = createServer(async (req, res) => {
  ...
  // .ts 원본은 타입스크립트 — 브라우저 실행 전 구문만 변환한다 (타입 제거, import 문은 유지)
  res.end(ext === ".ts" ? transpileTs(raw) : raw);
});

/** 타입스크립트 소스를 자바스크립트로 구문 변환한다 (Bun 내장 트랜스파일러 — loader: "ts")
 * @param {Buffer} source .ts 파일 내용
 * @returns {string} 브라우저 실행 가능한 JS
 */
function transpileTs(source) {
  if (typeof Bun === "undefined") throw new Error("verify.mjs는 TS 변환을 위해 bun으로 실행해야 합니다");
  return tsTranspiler.transformSync(source.toString("utf8"));
}
