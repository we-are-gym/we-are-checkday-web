...
export const TPL = {
	...
	recordRow({ id, session, date, total, max }) {
		return `
			<div class="record-row" data-record-id="${id}" tabindex="0" role="link" aria-label="${escapeHtml(session)} ${escapeHtml(date)} 총점 ${total}/${max}">
				...
				<div style="flex: 1; text-align: right"><button type="button" class="btn btn-sm btn-danger" data-del-record="${id}" aria-label="기록 삭제">삭제</button></div>
			</div>`;
	},
	...
	/**
	 * 도움말 모달 (app-help 컴포넌트 내부용 — 네이티브 <dialog>)
