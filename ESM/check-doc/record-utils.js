...
/**
 * ID로 체크기록 1건 조회
 * @param {import("@infra/store.js").CheckRecord[]} records 체크기록 배열
 * @param {number} id 기록 고유 번호 (Mason API `checkdoc_ID`)
 * @returns {import("@infra/store.js").CheckRecord | undefined}
 */
 ...
 * @param {import("@infra/store.js").CheckRecord[]} records 체크기록 배열
 * @param {string} memberId 회원 ID (Mason API `member_ID`)
 * @returns {import("@infra/store.js").CheckRecord[]}
 */
export function getRecordsByMember(records, memberId) {
	return records.filter(r => r.memberId === memberId).sort((a, b) => new Date(a.date) - new Date(b.date));
}

/**
 * 회원별 체크기록 건수 맵 (memberId → 기록 수)
 * @param {Array<{ memberId: string }>} records 체크기록 배열
 * @returns {Map<string, number>} 회원 id → 기록 건수 (기록 없는 회원은 키 부재)
 */
