...
| 모듈              | 내보내기                                           | 상태                                        | 저장 키               |
| ----------------- | -------------------------------------------------- | ------------------------------------------- | --------------------- |
| `member-store.js` | `memberStore`, `addMember`, `updateMember`                  | 회원 목록 6명 시드 (김씨 2인 포함)·등록/정보 갱신 헬퍼 | `checkday.members.v1` |
...
| 모듈                 | 내보내기                                                                                                        | 용도                                                            | 의존                                                                                                      |
| -------------------- | --------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| `assessment-data.js` | `ASSESSMENT_ITEMS`, `ASSESSMENT_ITEMS_FULL`, `ASSESSMENT_ITEMS_BASIC5`, `itemsForRecord` | 움직임 평가 7개·8개(VO₂) 항목 + 체크기록 작성용 5항목 + 기록별 항목 선택 헬퍼 | 없음                                                                                                      |
...
- 회원·체크기록은 **관찰자 패턴 스토어**(`createStore`)로 관리한다. 화면은 `subscribe`로 구독해 상태가 바뀌면 재렌더링된다.
  ...
  - `record.duration.payload` — `{ name, session(회차, 예: "3회차"), trainer, ib, ibComment, scores[N], evalData[N], goals, goalMemo, feedbacks, consultMemo }` — N은 항목 수(체크기록 작성 5·레거시 8)
