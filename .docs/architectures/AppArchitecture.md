...
| `export-image.js`     | `exportMemberDetailPNG`, `downloadPdf`, `pngFileName`             | 화면 PNG 캡처(html2canvas 벤더)·PDF 다운로드                      | `api-client`, `member-store`, `member-utils`, `@vendor/html2canvas`, `@shared/components/toast`, `utils-dom`                                                                             |
