// 파일 용도: 세션 리포트 — 결과 요약 HTML 조립·클립보드 복사 (checkday 공용)
...
export class SessionReport {
	...
	getEvalLines(prefix) {
		...
		return getEvals().map((e, i) => {
			...
			return formatEvalLine(e.name, getScore(i), checked, memo, prefix);
		});
	}
	...
	openModal() {
		...
		const evalLines = this.getEvalLines("");
    ...
    <div class="rline"><div class="rlabel">움직임 총점</div><div>${tot}/${getMax()}점</div></div>
