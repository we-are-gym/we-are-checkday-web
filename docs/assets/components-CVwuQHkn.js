import{t as e}from"./app-header-BTjZ8e3c.js";var t=({variant:e,disabled:t,loading:n,type:r,ariaLabel:i})=>{let a=`btn btn-${e}${t?` disabled`:``}${n?` loading`:``}`,o={};i&&(o.label=i),t&&(o.disabled=`true`),n&&(o.busy=`true`);let s=``;for(let[e,t]of Object.entries(o))s+=` aria-${e}="${t}"`;return`
			<button type="${r}" class="${a}"${s} ${t?`disabled`:``}>
				${n?`<span class="btn-spinner" aria-hidden="true"></span>`:``}
				<span class="btn-text"><slot>버튼</slot></span>
			</button>`};e({tag:`ui-button`,props:{variant:{type:String,default:`primary`},disabled:{type:Boolean,default:!1},loading:{type:Boolean,default:!1},type:{type:String,default:`button`},ariaLabel:{type:String,default:``}},render(){return t(this._getProps())},onConnect(){this.addEventListener(`click`,e=>{(this._props.disabled||this._props.loading)&&(e.preventDefault(),e.stopPropagation()),this.emit(`click`,{originalEvent:e})})}});var n=({href:e,variant:t,disabled:n,ariaLabel:r,target:i,rel:a})=>{let o=`btn btn-${t} link-btn${n?` disabled`:``}`,s={};r&&(s.label=r),n&&(s.disabled=`true`);let c=``;for(let[e,t]of Object.entries(s))c+=` aria-${e}="${t}"`;let l=i?` target="${i}"`:``,u=a?` rel="${a}"`:``;return`
			<a class="${o}"${n?``:` href="${e}"`}${l}${u}${c} role="button" tabindex="${n?-1:0}">
				<span class="btn-text"><slot>링크 버튼</slot></span>
			</a>`};e({tag:`ui-link-button`,props:{href:{type:String,default:`#`},variant:{type:String,default:`primary`},disabled:{type:Boolean,default:!1},ariaLabel:{type:String,default:``},target:{type:String,default:``},rel:{type:String,default:``}},render(){return n(this._getProps())},onConnect(){this.addEventListener(`click`,e=>{this._props.disabled&&(e.preventDefault(),e.stopPropagation()),this.emit(`click`,{originalEvent:e})}),this.addEventListener(`keydown`,e=>{this._props.disabled||(e.key===`Enter`||e.key===` `)&&(e.preventDefault(),!this._props.disabled&&this._props.href&&this._props.href!==`#`&&(window.location.href=this._props.href))})}});var r=({header:e,subHeader:t,collapsible:n,expanded:r,footer:i,variant:a,ariaLabel:o,ariaDescribedBy:s},{elementId:c,hasFooterSlot:l})=>{let u=`card-${c||`auto`}`,d=e||t,f=i||l,p={};o&&(p.label=o),s&&(p.describedby=s),n&&(p.expanded=String(r));let m=``;for(let[e,t]of Object.entries(p))m+=` aria-${e}="${t}"`;return`
			<div class="card-panel card-${a}${n?` collapsible`:``}${r?` expanded`:` collapsed`}"${m}>
				...
				`:``}
			</div>`};e({tag:`ui-card-panel`,props:{header:{type:String,default:``},subHeader:{type:String,default:``},collapsible:{type:Boolean,default:!1},expanded:{type:Boolean,default:!0},footer:{type:String,default:``},variant:{type:String,default:`default`},ariaLabel:{type:String,default:``},ariaDescribedBy:{type:String,default:``}},render(){return this._hasFooterSlot=this.querySelector(`[slot='footer']`)!==null,r(this._getProps(),{elementId:this.id,hasFooterSlot:this._hasFooterSlot})},onConnect(){this.addEventListener(`click`,e=>{this._props.collapsible&&e.target.closest(`[data-action='toggle']`)&&(this.setProp(`expanded`,!this._props.expanded),this.emit(`toggle`,{expanded:this._props.expanded}))}),this.addEventListener(`keydown`,e=>{this._props.collapsible&&(e.key===`Enter`||e.key===` `)&&e.target.closest(`[data-action='toggle']`)&&(e.preventDefault(),this.setProp(`expanded`,!this._props.expanded),this.emit(`toggle`,{expanded:this._props.expanded}))})}});var i=({checked:e,indeterminate:t,disabled:n,label:r,value:i,name:a,ariaLabel:o,ariaDescribedBy:s},{elementId:c})=>{let l={};o&&(l.label=o),s&&(l.describedby=s),l.checked=t?`mixed`:String(e),n&&(l.disabled=`true`);let u=``;for(let[e,t]of Object.entries(l))u+=` aria-${e}="${t}"`;let d=`checkbox-${c||`auto`}`;return`
			...
			</div>`};e({tag:`ui-score-controller`,props:{score:{type:Number,default:0},max:{type:Number,default:3},min:{type:Number,default:0},dots:{type:Number,default:4},index:{type:Number,default:0},interactive:{type:Boolean,default:!0},ariaLabel:{type:String,default:``},ariaDescribedBy:{type:String,default:``},showValue:{type:Boolean,default:!0}},render(){return o(this._getProps())},onConnect(){this._props.interactive&&(this.addEventListener(`click`,e=>{let t=e.target.closest(`.score-btn`);if(!t)return;let n=Number(t.dataset.delta);this._adjustScore(n)}),this.addEventListener(`keydown`,e=>{this._props.interactive&&(e.key===`ArrowRight`||e.key===`ArrowUp`?(e.preventDefault(),this._adjustScore(1)):(e.key===`ArrowLeft`||e.key===`ArrowDown`)&&(e.preventDefault(),this._adjustScore(-1)))}))},_adjustScore(e){let{score:t,min:n,max:r}=this._props,i=Math.max(n,Math.min(r,t+e));i!==t&&(this.setProp(`score`,i),this.emit(`adjust`,{score:i,delta:e,index:this._props.index}))}});var s=({score:e,max:t,count:n,size:r,interactive:i,ariaLabel:a,ariaDescribedBy:o,prefix:s},{elementId:c})=>{let l=Math.max(0,Math.min(t,e)),u=`dots-${s||c||`auto`}`,d={};a&&(d.label=a),o&&(d.describedby=o),d.role=`group`,d[`aria-label`]=`점수 ${l}점 / ${t}점`;let f=``;for(let[e,t]of Object.entries(d))f+=` aria-${e}="${t}"`;return`
			<div class="score-dots ${`dots-${r}`}${i?` interactive`:``}" id="${u}"${f}>
				${Array.from({length:n},(e,t)=>`
					...
					></span>
				`).join(``)}
			</div>`};e({tag:`ui-score-dots`,props:{score:{type:Number,default:0},max:{type:Number,default:3},count:{type:Number,default:4},size:{type:String,default:`md`},interactive:{type:Boolean,default:!1},ariaLabel:{type:String,default:``},ariaDescribedBy:{type:String,default:``},prefix:{type:String,default:``}},render(){return s(this._getProps(),{elementId:this.id})},onConnect(){this._props.interactive&&(this.addEventListener(`click`,e=>{let t=e.target.closest(`.dot`);if(!t)return;let n=Number(t.dataset.score);this.setProp(`score`,n),this.emit(`change`,{score:n,index:n})}),this.addEventListener(`keydown`,e=>{let t=e.target.closest(`.dot`);if(t){if(e.key===`Enter`||e.key===` `){e.preventDefault();let n=Number(t.dataset.score);this.setProp(`score`,n),this.emit(`change`,{score:n,index:n})}else e.key===`ArrowRight`?(e.preventDefault(),t.nextElementSibling?.focus()):e.key===`ArrowLeft`&&(e.preventDefault(),t.previousElementSibling?.focus())}}))}});var c=({data:e,width:t,height:n,color:r,background:i,showArea:a,showPoints:o,pointRadius:s,lineWidth:c,padding:l,baseline:u,ariaLabel:d,ariaDescribedBy:f})=>{if(!e||e.length===0)return`<div class="sparkline-empty" style="width: ${t}px; height: ${n}px;" aria-hidden="true">데이터 없음</div>`;let p=e.filter(e=>typeof e==`number`&&!isNaN(e));if(p.length===0)return`<div class="sparkline-empty" style="width: ${t}px; height: ${n}px;" aria-hidden="true">유효한 데이터 없음</div>`;let m=Math.min(...p),h=Math.max(...p),g=h-m||1,_=(t-l*2)/(p.length-1||1),v=p.map((e,t)=>({x:l+t*_,y:n-l-(e-m)/g*(n-l*2),value:e})),y=u!==null&&u>=m&&u<=h?n-l-(u-m)/g*(n-l*2):null,b=v.map((e,t)=>`${t===0?`M`:`L`} ${e.x.toFixed(2)} ${e.y.toFixed(2)}`).join(` `),x=``;a&&(x=[`M ${v[0].x} ${n-l}`,...v.map(e=>`L ${e.x} ${e.y}`),`L ${v[v.length-1].x} ${n-l}`,`Z`].join(` `));let S=o?v.map(e=>`
				<circle
					cx="${e.x.toFixed(2)}"
					cy="${e.y.toFixed(2)}"
					r="${s}"
		...
		>
			...
			${t?`<span class="tag-remove" aria-hidden="true">×</span>`:``}
				...
				<div class="text-box-wrapper${s?` disabled`:``}${c?` readonly`:``}${_?` has-error`:``}">
					...
					${_?`<div class="text-box-error" id="${S}" role="alert">${_}</div>`:``}
					${a>0?`<div class="text-box-counter" aria-hidden="true"><span class="current">${e.length}</span> / ${a}</div>`:``}
				</div>`};e({tag:`ui-text-box`,props:{value:{type:String,default:``},placeholder:{type:String,default:``},multiline:{type:Boolean,default:!1},rows:{type:Number,default:4},cols:{type:Number,default:50},maxLength:{type:Number,default:0},minLength:{type:Number,default:0},disabled:{type:Boolean,default:!1},readonly:{type:Boolean,default:!1},required:{type:Boolean,default:!1},spellcheck:{type:Boolean,default:!1},autocomplete:{type:String,default:`off`},ariaLabel:{type:String,default:``},ariaDescribedBy:{type:String,default:``},ariaInvalid:{type:Boolean,default:!1},label:{type:String,default:``},hint:{type:String,default:``},error:{type:String,default:``},autoResize:{type:Boolean,default:!1}},render(){return d(this._getProps(),{elementId:this.id})},onConnect(){let e=this.querySelector(`.text-box`);if(e){if(e.addEventListener(`input`,e=>{this.setProp(`value`,e.target.value),this.emit(`input`,{value:this._props.value,originalEvent:e})}),e.addEventListener(`change`,e=>{this.emit(`change`,{value:this._props.value,originalEvent:e})}),e.addEventListener(`blur`,e=>{this.emit(`blur`,{value:this._props.value,originalEvent:e})}),e.addEventListener(`focus`,e=>{this.emit(`focus`,{value:this._props.value,originalEvent:e})}),this._props.multiline&&this._props.autoResize){let t=()=>{e.style.height=`auto`,e.style.height=`${e.scrollHeight}px`};e.addEventListener(`input`,t),setTimeout(t,0)}if(this._props.maxLength>0){let t=this.querySelector(`.text-box-counter .current`);t&&e.addEventListener(`input`,()=>{t.textContent=this._props.value.length})}}}});