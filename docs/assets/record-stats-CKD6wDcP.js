import{t as e}from"./app-header-CObFATf5.js";import{n as t}from"./utils-array-Dlk6KB1T.js";import{i as n}from"./assessment-data-D8np2VSY.js";var r=[{key:`w`,label:`체중 (kg)`},{key:`m`,label:`골격근량 (kg)`},{key:`fat`,label:`체지방량 (kg)`},{key:`bmi`,label:`BMI`},{key:`bfp`,label:`체지방률 (%)`},{key:`bmr`,label:`기초대사량 (kcal)`},{key:`vis`,label:`내장지방`}];function i(e){return(e.scores||[]).length*3}function a(e,{width:t=260,height:n=68}={}){let r=e.filter(e=>!Number.isNaN(parseFloat(e))).map(Number);if(r.length===0)return`<span class="spark-empty">기록 없음</span>`;if(r.length===1)return`<span class="spark-empty">${r[0]} · 첫 회차만 기록됨</span>`;let i=Math.min(...r),a=Math.max(...r)-i||1,o=n-20-16,s=(t-20)/(r.length-1),c=r.map((e,t)=>[10+t*s,20+o*(1-(e-i)/a)]);return`
