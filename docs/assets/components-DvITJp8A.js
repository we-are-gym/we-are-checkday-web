import{t as e}from"./app-header-QCOnakgs.js";import"./data-table-DIx-wW7A.js";var t=({variant:e,disabled:t,loading:n,type:r,ariaLabel:i})=>{let a=`btn btn-${e}${t?` disabled`:``}${n?` loading`:``}`,o={};i&&(o.label=i),t&&(o.disabled=`true`),n&&(o.busy=`true`);let s=``;for(let[e,t]of Object.entries(o))s+=` aria-${e}="${t}"`;return`
			...
			</div>`};e({tag:`ui-score-controller`,props:{score:{type:Number,default:0},max:{type:Number,default:3},min:{type:Number,default:0},dots:{type:Number,default:4},index:{type:Number,default:0},interactive:{type:Boolean,default:!0},ariaLabel:{type:String,default:``},ariaDescribedBy:{type:String,default:``},showValue:{type:Boolean,default:!0}},render(){return a(this._getProps())},onConnect(){this._props.interactive&&(this.addEventListener(`click`,e=>{let t=e.target.closest(`.score-btn`);if(!t)return;let n=Number(t.dataset.delta);this._adjustScore(n)}),this.addEventListener(`keydown`,e=>{this._props.interactive&&(e.key===`ArrowRight`||e.key===`ArrowUp`?(e.preventDefault(),this._adjustScore(1)):(e.key===`ArrowLeft`||e.key===`ArrowDown`)&&(e.preventDefault(),this._adjustScore(-1)))}))},_adjustScore(e){let{score:t,min:n,max:r}=this._props,i=Math.max(n,Math.min(r,t+e));i!==t&&(this.setProp(`score`,i),this.emit(`adjust`,{score:i,delta:e,index:this._props.index}))}});var o=({score:e,max:t,count:n,size:r,interactive:i,ariaLabel:a,ariaDescribedBy:o,prefix:s},{elementId:c})=>{let l=Math.max(0,Math.min(t,e)),u=`dots-${s||c||`auto`}`,d={};a&&(d.label=a),o&&(d.describedby=o),d.role=`group`,d[`aria-label`]=`점수 ${l}점 / ${t}점`;let f=``;for(let[e,t]of Object.entries(d))f+=` aria-${e}="${t}"`;return`
